...
get_user_id() {
	...
	local user_id
	user_id=$(docker exec "$smtp_container" sqlite3 /app/data/databases/shared.db "
...
get_role_id() {
	...
	local role_id
	role_id=$(docker exec "$smtp_container" sqlite3 /app/data/databases/shared.db "
...
add_user_to_role() {
	...
	local exists
	exists=$(docker exec "$smtp_container" sqlite3 /app/data/databases/shared.db "
	...
	if docker exec "$smtp_container" sqlite3 /app/data/databases/shared.db "
...
remove_user_from_role() {
	...
	local rows_affected
	rows_affected=$(docker exec "$smtp_container" sqlite3 /app/data/databases/shared.db "
            SELECT changes();
        " 2>/dev/null | tr -d '\n\r')
